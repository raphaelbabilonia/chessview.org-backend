...
// Populates the connected MongoDB with demo data. Assumes an active mongoose
// connection (does NOT connect or disconnect). Used by the seed CLI and by the
// test harness (which points it at an in-memory MongoDB). Returns the number of
// events created.
const seedDatabase = async () => {
  ...
  const [, , nora, paolo, sara, tommaso] = players;
  const [openRound] = await Round.create([
    { event: openCuneo._id, section: openSection._id, number: 1, name: "Round 1", status: "published", startsAt: new Date("2026-07-11T09:00:00.000Z") }
  ]);

  const [ada, bruno] = players;
  await Pairing.create([
    { event: openCuneo._id, section: openSection._id, round: openRound._id, boardNumber: 1, whitePlayer: ada._id, blackPlayer: bruno._id, result: "pending" },
